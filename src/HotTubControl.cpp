...
void loop() {
	...
	// If pump on for 30 seconds and no pressure, turn pump off, change lastPressure millis so that the pump tries to run again in 30 seconds
	if(digitalRead(PUMPLOW) == HIGH && !statusPressure && currentMillis - lastPumpStartMillis > 150000){
		digitalWrite(PUMPLOW,LOW);
		lastPressureMillis += 150000;
	}
...
void callback(char *topic, byte *payload, unsigned int length){
	...
	if (String(topic) == String(topicPrefix + "reboot")){
		bootstrapManager.publish(tubStatusTopic.c_str(), "\"offline\"", false);
